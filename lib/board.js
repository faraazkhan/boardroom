...
exports.findBoardAllowEmpty = function( boardName, callback ) {
  withCollection( 'boards', function( collection ) {
    collection.findOne({ name: boardName }, errorWrapper(function (board) {
      callback(board);
    }));
  });
};
...
exports.createGroup = function( boardName, name, cardIds, callback ) {
  var group = {name: name, cardIds: cardIds};
  var groupId = new BSON.ObjectID();
  var groupWithId = {_id: groupId, name: name, cardIds: cardIds};
  var update = {$set: {}};
  update['$set']['groups.' + groupId] = group;

  withCollection('boards', function(boards) {
    boards.update({name: boardName}, update, callback ? {safe:true} : {}, errorWrapper(function () { callback(groupWithId); }));
  });
}

exports.updateGroup = function( boardName, _id, cardIds, callback ) {
  var update = {$set: {}};
